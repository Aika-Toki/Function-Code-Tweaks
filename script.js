...
function generateLogFunction() {
	if (Array.from(document.querySelectorAll("#settings-log>div>input")).filter((e) => e.value.length == 0).length == 0) {
		let itemName = document.querySelector("#input_settings-log_item-name").value;
		let logId =
			document.querySelector("#input_settings-log_log-block-id").value.indexOf(":") == -1
				? "minecraft:" + document.querySelector("#input_settings-log_log-block-id").value
				: document.querySelector("#input_settings-log_log-block-id").value;
		let leavesId =
			document.querySelector("#input_settings-log_leave-block-id").value.indexOf(":") == -1
				? "minecraft:" + document.querySelector("#input_settings-log_leave-block-id").value
		...
		let code = [codeLogTemplate(itemName, logId, "y"), codeLogTemplate(itemName, logId, "x"), codeLogTemplate(itemName, logId, "z"), codeLeaveTemplate(itemName, leavesId)].join("\n\n");
...
function generateOreFunction() {
	if (Array.from(document.querySelectorAll("#settings-ore>div>input")).filter((e) => e.value.length == 0).length == 0) {
		let itemName = document.querySelector("#input_settings-ore_item-name").value;
		let oreId =
			document.querySelector("#input_settings-ore_ore-block-id").value.indexOf(":") == -1
				? "minecraft:" + document.querySelector("#input_settings-ore_ore-block-id").value
...
function checkedRadio(name) {
	return Array.from(document.querySelectorAll("input[type=radio][name=" + name + "]")).filter((e) => e.checked).length == 0
		? undefined
		: Array.from(document.querySelectorAll("input[type=radio][name=" + name + "]")).filter((e) => e.checked)[0].value;
}
function goTop() {
